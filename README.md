...
在群組中 `@` 機器人、輸入 `/help` 或 `/說明` 會顯示功能選單；手機版點選新增功能會開啟鍵盤並預填 `/h `、`/m ` 或 `/n `，點選查詢功能則直接顯示結果。查詢結果每頁顯示 5 筆，依日期由新到舊排列，並可直接點擊內容右側的「完成」按鈕。新增或完成成功時不另行回覆，能取得 LINE 已讀憑證時會標記為已讀；格式錯誤、權限不足或找不到編號時仍會回覆提示。只有交班資訊會在查詢結果顯示登錄者姓名；缺換藥與公告保留後端稽核紀錄，但不在群組查詢內容顯示姓名。`/open` 會顯示事項已存在天數，超過 7 天時提示可能過期。
