...
在群組中 `@` 機器人、輸入 `/help` 或 `/說明` 會顯示功能選單；手機版點選新增功能會開啟鍵盤並預填 `/h `、`/m `、`/n ` 或 `/e `，點選查詢功能則直接顯示結果。查詢結果每頁顯示 5 筆，依日期由新到舊排列；交班可點「已處理」，其餘分類可點「刪除」。公告內容包含 `http` 或 `https` 網址時，查詢結果會提供「開啟連結」按鈕。新增或處理成功時不另行回覆，能取得 LINE 已讀憑證時會標記為已讀；格式錯誤、權限不足或找不到編號時仍會回覆提示。只有交班資訊會在查詢結果顯示登錄者姓名；其他分類保留後端稽核紀錄，但不在群組查詢內容顯示姓名。
